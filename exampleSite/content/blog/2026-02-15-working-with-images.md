...
External URLs work but Hugo cannot read their dimensions. The image renders without `width`/`height` attributes. This is correct behaviour — no dimensions are better than wrong dimensions.
...
Always write descriptive alt text. It is read by screen readers and displayed when the image fails to load. An empty `alt=""` is correct for purely decorative images — it tells screen readers to skip the element.
