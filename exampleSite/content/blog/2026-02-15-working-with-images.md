...
The `imageMode` param controls how Markdown images are rendered. Set it site-wide in `hugo.toml` or override it per page in front matter.
...
In `embed` mode, the first image on a page (`Ordinal 0`) is treated as a likely above-the-fold image. It is rendered with `loading="eager"` and `fetchpriority="high"` to improve Largest Contentful Paint (LCP). All subsequent images use `loading="lazy"`. This is automatic, no configuration required.
...
External URLs work but Hugo cannot read their dimensions. The image renders without `width`/`height` attributes. This is correct behaviour, as no dimensions are better than wrong dimensions.
...
Always write descriptive alt text. It is read by screen readers and displayed when the image fails to load. An empty `alt=""` is correct for purely decorative images, as it tells screen readers to skip the element.
